...
def BMI():
    ...
    #you can change value as different countries have different bmi indexes 

    if bmi<=18.5:
        label2.config(text="Underweight!")
        label3.config(text="You have lowwer weight than normal!")
    elif bmi>18.5 and bmi<=25:
        label2.config(text="Normal!")
        label3.config(text="You are Healthy <3 !")
    elif bmi>25 and bmi<=30:
        label2.config(text="Overweight !!")
        label3.config(text="You are slightly Overweight !")
    else:
        label2.config(text="Obes!")
        label3.config(text="You are obese! A docctor Advice is must!")
...
#Command to cgange Background of the scale
style=ttk.Style()
...
label1=Label(root,font="arial 60 bold",bg="light blue",fg="#fff")
